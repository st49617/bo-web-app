...
public class NutSheetProcessor extends AbstractSheetProcessor
{
    ...
    public List<Item> disintegrateIntoItem(int rowIdx, List<String> rowData, Supplier supplier) {
        if (!rowData.isEmpty())
        {
            ...
            if (values.length >= 3)
            {
                parseOneItem(supplier, values, itemsList, 0);
            }
            if (values.length >= 9)
            {
                parseOneItem(supplier, values, itemsList, 5);
            }
            return itemsList;
        }
        else {
    ...
    private void parseOneItem(Supplier supplier, String[] values, List<Item> itemsList, int startColumnIdx) {
